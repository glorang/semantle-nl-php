...
function solveStory(guesses, puzzleNumber, won, hints_used) {
    ...
    let describe = function(similarity, percentile) {
        let out = `had een score van ${similarity.toFixed(2)}`;
        if (percentile) {
            out += ` (${percentile}/1000)`;
        }
        return out;
    ...
    if (!first_guess_in_top) {
        for (let entry of guesses_chrono) {
            ...
            if (percentile) {
                first_hit = ` Mijn eerste woord in de top 1000 was op gok #${guess_number}. `;
                break;
            }
        }
    }
...
`;
        ...
        $('#give-up-btn').addEventListener('click', function(event) {
            if (!gameOver) {
                if (confirm("Ben je zeker dat je wilt opgeven?")) {
                    endGame(false, true);
                }
            }
        });
    ...
    function updateGuesses() {
        let inner = `<tr><th id="chronoOrder">#</th><th id="alphaOrder">Woord</th><th id="similarityOrder">Score</th><th>Getting close?</th></tr>`;
    ...
    function endGame(won, countStats) {
        ...
        if (won) {
            ...
            response = `<p><b>Verloren! Het geheime woord is: ${secret}</b>.<br />Je kan alle dichtstbijzijnde woorden <a href="nearby_1k/${secretBase64}">hier</a> zien.</p>`;
        }
...
Stats: <br/>
<table>
<tr><th>Eerste spel (nummer):</th><td>${stats['firstPlay']}</td></tr>
<tr><th>Totaal aantal dagen gespeeld:</th><td>${totalGames}</td></tr>
<tr><th>Gewonnen:</th><td>${stats['wins']}</td></tr>
<tr><th>Win streak:</th><td>${stats['winStreak']}</td></tr>
<tr><th>Opgegeven:</th><td>${stats['giveups']}</td></tr>
